...
''
  ...
  input {
      ...
      ${if gestureSwipeFingers == "" then "" else "gesture-swipe-fingers ${gestureSwipeFingers}"}
      warp-mouse-to-focus
      focus-follows-mouse
  }
  layout {
      gaps ${if gaps == "" then "10" else gaps}
  ...
  window-rule {
      geometry-corner-radius ${if radius == "" then "10" else radius}
  ...
  ${extraKeybinds}
      ...
      Mod+S hotkey-overlay-title="Take a Screenshot of the selected area" {
          spawn-sh r#"grim -g "$(slurp -d)" - | wl-copy"#
      }
