...
`rb` loads secrets directly from `secrets/secrets.nix`. `.gitattributes` keeps
files under `secrets/` encrypted with `git-crypt` when committed. Unlock the
clone before activation:
